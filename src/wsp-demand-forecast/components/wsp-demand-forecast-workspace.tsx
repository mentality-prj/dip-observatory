...
import { Activity, BrainCircuit, CheckCircle2, LineChart, Play, RotateCcw, Target, Zap } from "lucide-react";
...
function ForecastChart({ values, playing }: { values: number[]; playing: boolean }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/8 bg-slate-950/70 p-4">
      {playing && <div className="pointer-events-none absolute inset-0 animate-pulse bg-gradient-to-r from-transparent via-cyan-300/5 to-transparent" />}
  ...
  return (
    <main className="min-h-screen bg-slate-950 px-4 py-8 text-white md:px-8">
      <div className="mx-auto max-w-[1500px] space-y-6">
        ...
        <section className={cn("overflow-hidden rounded-3xl border p-5 transition-all duration-700 md:p-7", step >= 3 ? "border-emerald-300/30 bg-emerald-300/[0.06]" : "border-white/8 bg-white/[0.025]")}><div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-start gap-4"><div className={cn("flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl transition-all duration-500", step >= 3 ? "bg-emerald-300 text-slate-950" : "bg-white/8 text-cyan-300")}>{step >= 3 ? <CheckCircle2 className="h-6 w-6" /> : <Target className="h-6 w-6" />}</div><div><div className="text-xs font-semibold uppercase tracking-[0.22em] text-cyan-300">{copy.decision}</div><h2 className="mt-2 text-xl font-semibold md:text-2xl">{step >= 3 ? (scenario === "shock" ? copy.monitor : copy.capacity) : "Awaiting model execution"}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">{step >= 3 ? `${copy.why}: ${direction} · ${change >= 0 ? "+" : ""}${change.toFixed(1)}% forecast shift over ${horizon}.` : "Run the simulation to propagate the market signal through forecast uncertainty into a planning decision."}</p></div></div><Badge variant={step >= 3 ? "emerald" : "neutral"}>{copy.validated}</Badge></div></section>
        <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-white/7 pt-4 text-[10px] uppercase tracking-[0.16em] text-slate-600"><span>WSP demand intelligence · demonstration only</span><Button size="sm" variant="ghost" onClick={reset}><RotateCcw className="mr-2 h-3.5 w-3.5" />{copy.reset}</Button></footer>
      </div>
    </main>
  );
}
