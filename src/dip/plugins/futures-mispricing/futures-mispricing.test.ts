...
import { DEFAULT_CONFIG, runFuturesMispricingPlugin } from '@/dip/plugins/futures-mispricing'
import {
  computeOverallSlope,
  computeLocalSlope,
  computeCurvature,
...
function makePoints(data: Array<{ contract: string; ordinal: number; price: number }>): ForwardCurvePoint[] {
  return data.map((d) => ({
    contract: d.contract,
...
function makePluginRequest(overrides: Partial<FuturesMispricingRequest> = {}): FuturesMispricingRequest {
  return {
...
describe('data integrity', () => {
  test('snapshot does not contain the 558 PLN outcome price', () => {
    for (const point of EIDOS_MARKET_SNAPSHOT.points) {
      assert.notEqual(point.price, 558, `Point ${point.contract} must not contain the post-decision 558 PLN price`)
    }
  })

  test('outcome is in a separate sealed object', () => {
    assert.equal(EIDOS_Q1_2027_OUTCOME._label, 'SUBSEQUENT_OUTCOME_NOT_AVAILABLE_AT_DECISION_TIME')
    assert.equal(EIDOS_Q1_2027_OUTCOME.outcome.referencePrice, 558)
  })

  test('dataset contains mandatory contracts', () => {
    const requiredContracts = ['Q4-2026', 'Q1-2027', 'Q2-2027', 'Q3-2027', 'Cal-2027', 'Cal-2028', 'Cal-2029']
...
describe('calendar spreads', () => {
  test('spread to previous: target is more expensive than predecessor', () => {
    ...
    assert.ok(Math.abs(spreadToPrevious - (479 - 508)) < 1e-9, `Expected -29, got ${spreadToPrevious}`)
  })
  ...
  test('spread to next: target is cheaper than successor', () => {
    ...
    // 479 - 498 = -19
    assert.ok(Math.abs(spreadToNext - (479 - 498)) < 1e-9, `Expected -19, got ${spreadToNext}`)
  })
  ...
  test('annual spread EIDOS snapshot: Q1-2027 is below Cal-2027', () => {
    const spread = computeAnnualSpread(EIDOS_MARKET_SNAPSHOT.points, 'Q1-2027')
...
describe('relative valuation', () => {
  ...
  test('normalised deviation EIDOS: Q1-2027 has negative deviation (cheap)', () => {
    const dev = computeNormalisedDeviation(EIDOS_MARKET_SNAPSHOT.points, 'Q1-2027')
    assert.ok(dev < 0, `Q1-2027 should have negative normalised deviation; got ${dev}`)
  })

  test('relative position EIDOS: Q1-2027 trades at discount vs local curve', () => {
    const pos = computeRelativePosition(EIDOS_MARKET_SNAPSHOT.points, 'Q1-2027')
    assert.ok(pos.vsLocalCurve < 0, `Q1-2027 should be below local interpolation; got ${pos.vsLocalCurve}`)
    assert.ok(pos.vsAnnual < 0, `Q1-2027 should be below annual proxy; got ${pos.vsAnnual}`)
  })
})
...
describe('uncertainty model', () => {
  test('historical dispersion: constant series returns 0', () => {
    const obs = [{ price: 100 }, { price: 100 }, { price: 100 }]
  ...
  test('local curve dispersion is positive for EIDOS snapshot', () => {
    const sigma = computeLocalCurveDispersion(EIDOS_MARKET_SNAPSHOT, 'Q1-2027')
  ...
  test('distance factor = 1 for evenly-spaced neighbouring contracts', () => {
    ...
    assert.ok(Math.abs(factor - 1) < 1e-9, `Expected distance factor 1, got ${factor}`)
  })
  ...
  test('combined uncertainty has positive half-width', () => {
    const sigma = computeHistoricalDispersion(EIDOS_Q1_2027_HISTORY)
    const { halfWidth } = computeCombinedUncertainty(sigma, EIDOS_MARKET_SNAPSHOT, 'Q1-2027')
  ...
  test('uncertainty range is symmetric around central estimate', () => {
    const valuation = buildUncertaintyRange(500, EIDOS_Q1_2027_HISTORY, EIDOS_MARKET_SNAPSHOT, 'Q1-2027')
  ...
  test('methodology string includes densityFactor for auditability', () => {
    const valuation = buildUncertaintyRange(500, EIDOS_Q1_2027_HISTORY, EIDOS_MARKET_SNAPSHOT, 'Q1-2027')
    assert.ok(valuation.methodology.includes('densityFactor='), 'methodology must report densityFactor')
  })
...
describe('hedge decision', () => {
  ...
  test('decision for expensive contract returns NO_ACTION', () => {
    ...
    const snapshot = makeSnapshot(pts)
    const obs = [
      { date: '2026-01-01', price: 455 },
      { date: '2026-03-01', price: 452 },
    ]
    const decision = computeHedgeDecision(snapshot, 'Q1-2027', obs, '2026-05-26')
  ...
  test('EIDOS case: valuation range lower > 0 and upper > lower', () => {
    ...
    assert.ok(decision.valuationRange.central > decision.valuationRange.lower, 'central must be above lower')
  })
...
describe('look-ahead bias protection', () => {
  test('decision function signature does not accept OutcomeData', () => {
    ...
    assert.equal(decision.entryPrice, 479, 'Entry price must be 479 (decision-time price), not 558 (outcome price)')
  })
  ...
  test('all historical observations are at or before decision date', () => {
    for (const obs of EIDOS_Q1_2027_HISTORY) {
      assert.ok(obs.date <= EIDOS_DECISION_DATE, `Observation ${obs.date} must not be after decision date`)
    }
  })
  ...
  test('outcome is not reachable from the market snapshot', () => {
    ...
    for (const point of EIDOS_MARKET_SNAPSHOT.points) {
      assert.notEqual(point.price, 558, `Snapshot must not contain the post-decision 558 PLN price`)
    }
    // Historical observations also must not contain 558
    for (const obs of EIDOS_Q1_2027_HISTORY) {
      assert.notEqual(obs.price, 558, `Historical obs must not contain the post-decision 558 PLN price`)
    }
  })
...
describe('plugin entry point', () => {
  ...
  test('filters post-decision observations at the plugin boundary', () => {
    const withFutureObservation = runFuturesMispricingPlugin(
      makePluginRequest({
        historicalObservations: [...EIDOS_Q1_2027_HISTORY, { date: '2026-06-01', price: 1000 }],
      })
    )
    ...
    assert.deepEqual(withFutureObservation.decisionTrace.uncertaintyRange, baseline.decisionTrace.uncertaintyRange)
    assert.deepEqual(withFutureObservation.decisionTrace.historicalDynamics, baseline.decisionTrace.historicalDynamics)
  })
})
...
describe('EIDOS historical case study regression (2026-05-26)', () => {
  ...
  test('EIDOS regression: calculated values match frozen snapshot', () => {
    const decision = computeHedgeDecision(EIDOS_MARKET_SNAPSHOT, 'Q1-2027', EIDOS_Q1_2027_HISTORY, '2026-05-26')
    ...
    assert.ok(decision.minimax.worstCaseLow <= decision.valuationRange.lower + 0.5)
    assert.ok(decision.minimax.worstCaseHigh >= decision.valuationRange.upper - 0.5)

    // Decision must be one of the valid signals
    assert.ok(['BUY', 'WATCH', 'NO_ACTION'].includes(decision.action), `Expected valid signal; got ${decision.action}`)

    // Robustness must be valid
    assert.ok(['HIGH', 'MEDIUM', 'LOW'].includes(decision.robustness))
