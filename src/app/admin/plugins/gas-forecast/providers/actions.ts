...
import { logGasForecastDiagnostic, testGasForecastProviderConnection } from '@/lib/gas-forecast-provider-client'
...
const entsogSchema = z
  ...
  .superRefine((input, context) => {
    const dateError = validateEntsogHistoricalDateRange({ from: input.from, to: input.to }, getTodayLocalDateIso())
...
export async function testGasForecastProviderAction(input: {
  ...
  try {
    ...
    const fallbackProviderId = GAS_FORECAST_PROVIDER_IDS.includes(input?.providerId as GasForecastProviderId)
