...
function validationFields(error:ZodError){return Object.fromEntries(error.issues.flatMap(issue=>{const field=issue.path[0];return typeof field==="string"?[[field,issue.code]]:[]}))}
export async function POST(request:NextRequest){
 const id=submissionId(),locale=request.headers.get("accept-language")?.slice(0,2)||"unknown";
 try{
  if(request.headers.get("content-length")&&Number(request.headers.get("content-length"))>MAX_BODY_BYTES)return NextResponse.json({error:"payload_too_large"},{status:413});
  ...
  const raw=await request.text(); if(Buffer.byteLength(raw,"utf8")>MAX_BODY_BYTES)return NextResponse.json({error:"payload_too_large"},{status:413});
  let json:unknown;try{json=JSON.parse(raw)}catch{return NextResponse.json({error:"malformed_request"},{status:400})}
  ...
  if(error instanceof ZodError)return NextResponse.json({error:"validation_failed",fields:validationFields(error)},{status:400});
