...
import { FuturesOpportunityDocumentation } from "@/eidos/components/futures-opportunity-documentation";
import { Badge } from "@/components/ui/badge";
import { buildLocalePath, type Locale } from "@/lib/observatory-i18n";
import { getEidosCopy } from "@/eidos/lib/eidos-i18n";

const OPPORTUNITY_ACTIVE_LOCALES = new Set(["en", "uk", "pl"]);
const OPPORTUNITY_FALLBACK_LOCALE: Locale = "en";

export function generateStaticParams() {
  return ["en", "uk", "pl"].map((locale) => ({ locale }));
}

export default async function FuturesOpportunityDocumentationPage({
  params,
}: {
  ...
  if (!OPPORTUNITY_ACTIVE_LOCALES.has(locale)) {
    redirect(
      buildLocalePath("/eidos/opportunity/documentation", OPPORTUNITY_FALLBACK_LOCALE),
    );
  }
        ...
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h1 className="mt-1 text-2xl font-semibold tracking-tight text-white md:text-3xl">
              {copy.opportunityDocumentation.title}
            </h1>
            <p className="mt-1 max-w-3xl text-sm text-slate-500">
              {copy.opportunityDocumentation.description}
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Link
              href={buildLocalePath("/eidos/opportunity", resolvedLocale)}
            ...
            >
              ...
              {copy.header.backToOpportunity}
            </Link>
          </div>
        </header>

        <FuturesOpportunityDocumentation locale={resolvedLocale} />
