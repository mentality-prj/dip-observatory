"use client";

import { Card, CardContent } from "@/components/ui/card";
import { getEidosCopy } from "@/eidos/lib/eidos-i18n";
import type { Locale } from "@/lib/observatory-i18n";

type Props = {
  locale: Locale;
  id?: string;
};

export function FuturesOpportunityDocumentation({ locale, id }: Props) {
  const copy = getEidosCopy(locale);

  return (
    <Card
      id={id}
      className="border-emerald-300/20 bg-[linear-gradient(180deg,rgba(6,20,14,0.96),rgba(3,10,7,0.98))]"
    >
      <CardContent className="grid gap-4 px-6 py-6 lg:grid-cols-2">
        {copy.opportunityDocumentation.sections.map((section) => (
          <section
            key={section.title}
            className="rounded-2xl border border-white/8 bg-white/4 p-4"
          >
            <h3 className="text-sm font-semibold uppercase tracking-[0.16em] text-emerald-100">
              {section.title}
            </h3>
            <p className="mt-2 text-sm leading-6 text-slate-300">
              {section.description}
            </p>
            <ul className="mt-3 flex list-disc flex-col gap-2 pl-5 text-sm text-slate-400">
              {section.bullets.map((bullet) => (
                <li key={bullet}>{bullet}</li>
              ))}
            </ul>
          </section>
        ))}
      </CardContent>
    </Card>
  );
}
