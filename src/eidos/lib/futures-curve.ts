...
export function computeStructuralValuation(
  ...
  // 70% local curve interpolation, 30% annual proxy
  const central = 0.7 * localInterpolated + 0.3 * annualProxy;
  ...
  // Uncertainty width = 1.5 × local dispersion, minimum 10 PLN
  const halfWidth = Math.max(1.5 * wSigma, 10);
