...
describe("1. baseline immutability", () => {
  test("PDR_PRE_DISRUPTION_SCENARIO is not mutated by engine runs", () => {
    ...
    const originalDisruption = JSON.stringify(
      PDR_PRE_DISRUPTION_SCENARIO.disruption,
    );
    runPreDisruption();
    runPreDisruption();
    assert.equal(
      JSON.stringify(PDR_PRE_DISRUPTION_SCENARIO.orders),
      originalOrders,
    );
    assert.equal(
      JSON.stringify(PDR_PRE_DISRUPTION_SCENARIO.disruption),
      originalDisruption,
    );
  });
  ...
  test("pre-disruption scenario has zero capacity reduction", () => {
    assert.equal(
      PDR_PRE_DISRUPTION_SCENARIO.disruption.capacityReductionFactor,
      0,
    );
  });
  ...
  test("disrupted scenario has full capacity reduction on LINE-B", () => {
    ...
    assert.equal(
      PDR_DISRUPTED_SCENARIO.disruption.capacityReductionFactor,
      1.0,
    );
...
describe("2. Machine B disruption changes capacity", () => {
  test("pre-disruption: all orders on Machine B can be scheduled on Day 1", () => {
    ...
    assert.ok(
      machineBDay1Tasks.length > 0,
      "Machine B should have tasks on Day 1 in pre-disruption",
    );
  });
  ...
  test("disrupted: no tasks can be placed on LINE-B Day 1 with factor=1.0", () => {
    ...
    assert.equal(
      machineBDay1Tasks.length,
      0,
      "LINE-B Day 1 should have no tasks when fully disrupted",
    );
  });
...
describe("3. affected orders identified", () => {
  ...
  test("exactly 3 orders are at risk in default 8h disruption", () => {
    ...
    assert.equal(
      atRisk.length,
      3,
      `Expected 3 orders at risk, got ${atRisk.length}: ${atRisk.join(", ")}`,
    );
  });
  ...
  test("no orders at risk in pre-disruption baseline", () => {
    ...
    assert.equal(
      atRisk.length,
      0,
      "No orders should be at risk before disruption",
    );
  });
});
...
describe("4. deadline risk recalculated", () => {
  test("KEEP_CURRENT violates RULE-CRITICAL-DEADLINE when disrupted", () => {
    ...
    assert.ok(keep);
    const critRule = keep!.constraintResults.find(
    ...
    assert.equal(
      critRule!.passed,
      false,
      "RULE-CRITICAL-DEADLINE must fail for KEEP_CURRENT when disrupted",
    );
  });
  ...
  test("KEEP_CURRENT passes RULE-CRITICAL-DEADLINE in pre-disruption", () => {
    ...
    assert.equal(
      critRule!.passed,
      true,
      "Critical deadline should pass in pre-disruption",
    );
  });
...
describe("5. alternative machines respected", () => {
  test("REDISTRIBUTE moves PDR-104 (PERGOLA) from LINE-B to LINE-C", () => {
    const result = runPdrEngine();
    const redis = result.strategies.find(
  ...
  test("REDISTRIBUTE keeps AWNING orders on LINE-B", () => {
    ...
    for (const id of awningOrders) {
      const task: ScheduledTask | undefined = redis!.schedule.find(
        (t) => t.orderId === id,
      );
      ...
      assert.equal(
        task!.lineId,
        "LINE-B",
        `${id} should stay on LINE-B in REDISTRIBUTE`,
      );
    }
  });
  ...
  test("machine compatibility constraints pass in recommended strategy", () => {
    ...
    assert.ok(rec);
    const compatRule = rec!.constraintResults.find(
    ...
    assert.equal(
      compatRule!.passed,
      true,
      "Machine compatibility must pass in recommended strategy",
    );
  });
});
...
describe("6. setup/changeover recalculated", () => {
  test("REDISTRIBUTE incurs setup hours from moving PDR-104 to LINE-C", () => {
    ...
    assert.ok(redis);
    const totalSetup = redis!.lineUtilization.reduce(
      (s, l) => s + l.setupHours,
      0,
    );
  ...
  test("RULE-SETUP is always evaluated and passes (informational)", () => {
    ...
    for (const strategy of result.strategies) {
      const setupRule = strategy.constraintResults.find(
        (r) => r.ruleId === "RULE-SETUP",
      );
      assert.ok(setupRule, `RULE-SETUP not found in ${strategy.strategyId}`);
      assert.equal(
        setupRule!.passed,
        true,
        "RULE-SETUP is informational and always passes",
      );
    }
  });
  ...
  test("setup cost uses €80/h rate from DEFAULT_COST_CONFIG", () => {
    ...
    assert.ok(rec);
    const setupHours = rec!.lineUtilization.reduce(
      (s, l) => s + l.setupHours,
      0,
    );
...
describe("7. overtime recalculated", () => {
  test("USE_OVERTIME shows non-zero overtime hours for PDR-104 on LINE-B Day 1", () => {
    ...
    assert.ok(lineBUtil);
    assert.ok(
      lineBUtil!.overtimeHours > 0,
      "LINE-B should have overtime hours when enabled",
    );
  });

  test("overtime cost is zero when overtime disabled", () => {
    const result = runPdrEngine({
      scenarioId: "PDR-NO-OT",
  ...
  test("buildPdrScenario passes overtime flag correctly", () => {
    const { scenario: withOT } = buildPdrScenario({
      ...BASELINE_DISRUPTION_WHAT_IF,
      overtimeAvailable: true,
    });
    const { scenario: noOT } = buildPdrScenario({
      ...BASELINE_DISRUPTION_WHAT_IF,
      overtimeAvailable: false,
    });
...
describe("8. delay cost recalculated", () => {
  test("KEEP_CURRENT has non-zero delay cost when disrupted", () => {
    ...
    assert.ok(keep);
    assert.ok(
      keep!.financialImpact.delayCost > 0,
...
describe("9. all alternatives evaluated", () => {
  test("engine evaluates exactly 5 strategies", () => {
    ...
    assert.equal(
      ...
      5,
      "Should evaluate exactly 5 strategies",
    );
  });
  ...
  test("KEEP_CURRENT and PRIORITIZE_URGENT are infeasible when disrupted", () => {
    ...
    const prio = result.strategies.find(
      (s) => s.strategyId === "PRIORITIZE_URGENT_ORDERS",
    );
    assert.equal(
      keep!.feasibility,
      "INFEASIBLE",
      "KEEP_CURRENT must be INFEASIBLE",
    );
    assert.equal(
      prio!.feasibility,
      "INFEASIBLE",
      "PRIORITIZE_URGENT must be INFEASIBLE",
    );
  });
  ...
  test("REDISTRIBUTE is feasible for 8h disruption", () => {
    ...
    assert.equal(
      redis!.feasibility,
      "FEASIBLE",
      "REDISTRIBUTE must be FEASIBLE for 8h disruption",
    );
  });
  ...
  test("USE_OVERTIME is feasible when overtime is enabled", () => {
    ...
    assert.equal(
      ot!.feasibility,
      "FEASIBLE",
      "USE_OVERTIME must be FEASIBLE when overtime enabled",
    );
  });
});
...
describe("11. financial totals correct", () => {
  ...
  test("totalFinancialImpact matches recommended strategy totalCost", () => {
    ...
    assert.ok(rec);
    assert.ok(
      Math.abs(result.totalFinancialImpact - rec!.financialImpact.totalCost) <
        1.0,
...
describe("12. trace matches rule results", () => {
  test("RULE-CRITICAL-DEADLINE trace evidence matches delayed status of PDR-104", () => {
    ...
    assert.ok(keep);
    const critTrace = keep!.constraintResults.find(
      (r) => r.ruleId === "RULE-CRITICAL-DEADLINE",
    );
    assert.ok(critTrace);
...
describe("13. reset restores baseline", () => {
  test("buildPdrScenario with BASELINE_DISRUPTION_WHAT_IF produces same result as PDR_DISRUPTED_SCENARIO", () => {
    ...
    const fromBuilder = runSchedulingEngine({
      scenario,
      costConfig: DEFAULT_COST_CONFIG,
    });
  ...
  test("machineBAvailable=true: no orders at risk", () => {
    ...
    assert.equal(
      atRisk.length,
      0,
      "No orders at risk when Machine B is available",
    );
  });

  test("sensitivity returns 4 entries for all disruption durations", () => {
    const sensitivity = computeDisruptionSensitivity(
      BASELINE_DISRUPTION_WHAT_IF,
    );
  ...
  test("sensitivity: all durations are feasible via REDISTRIBUTE (CRITICAL order always moves to LINE-C)", () => {
    ...
    for (const entry of sensitivity) {
      assert.equal(
        entry.feasible,
        true,
        `${entry.hours}h disruption should be feasible`,
      );
      assert.equal(
        entry.strategy,
        "Redistribute to Other Lines",
...
describe("14. schedule label semantics", () => {
  test("pre-disruption result: all Machine B orders are scheduled on time", () => {
    ...
    const rec = pre.strategies.find(
      (s) => s.strategyId === pre.recommendedStrategy,
    );
    ...
    for (const id of machineBIds) {
      const task: ScheduledTask | undefined = rec!.schedule.find(
        (t) => t.orderId === id,
      );
      ...
      assert.equal(
        task!.status,
        "ON_TIME",
        `${id} must be ON_TIME in pre-disruption (BEFORE DISRUPTION state)`,
      );
    }
  });
  ...
  test("disrupted result KEEP_CURRENT_SCHEDULE represents disruption without recovery (DISRUPTED PLAN)", () => {
    ...
    assert.equal(
      hasDelayed,
      true,
      "DISRUPTED PLAN must contain at least one delayed/not-scheduled order",
    );
  });
  ...
  test("disrupted result recommended strategy represents recovery plan (RECOVERY PLAN)", () => {
    ...
    const rec = disrupted.strategies.find(
      (s) => s.strategyId === disrupted.recommendedStrategy,
    );
    assert.ok(rec, "Recovery plan must exist");
    assert.notEqual(
      disrupted.recommendedStrategy,
      "KEEP_CURRENT_SCHEDULE",
    ...
    const keepDelays = (keep?.schedule ?? []).filter(
      (t) => t.status === "DELAYED" || t.status === "NOT_SCHEDULED",
    ).length;
    const recDelays = rec!.schedule.filter(
      (t) => t.status === "DELAYED" || t.status === "NOT_SCHEDULED",
    ).length;
    assert.ok(
      recDelays <= keepDelays,
      "Recovery plan must not have more delays than KEEP_CURRENT",
    );
  });
});
...
describe("15. machine B order count and risk classification", () => {
  test("exactly 4 orders are assigned to Machine B", () => {
    const machineBOrders = PDR_ORDERS.filter(
      (o) => o.defaultLineId === "LINE-B",
    );
    assert.equal(
      machineBOrders.length,
      4,
      "Must be exactly 4 orders on Machine B",
    );
    assert.deepEqual(
    ...
    assert.equal(
      ...
      4,
      "PDR_MACHINE_B_ORDER_IDS must contain 4 entries",
    );
  });
  ...
  test("exactly 3 orders are at risk (delayed/not-scheduled) in KEEP_CURRENT", () => {
    ...
    assert.equal(
      atRisk.length,
      3,
      `Expected 3 at-risk orders but got ${atRisk.length}: ${atRisk.join(", ")}`,
    );
  });
  ...
  test("exactly 1 Machine B order remains unaffected (on-time in KEEP_CURRENT)", () => {
    ...
    assert.equal(
      unaffected,
      1,
      `Expected 1 unaffected Machine B order but got ${unaffected}`,
    );
    // PDR-106 (deadline Day 2, 3h duration) should be the unaffected order
    const unaffectedId = PDR_MACHINE_B_ORDER_IDS.find(
      (id) => !atRisk.includes(id),
    );
    assert.equal(
      unaffectedId,
      "PDR-106",
      "PDR-106 should be the unaffected Machine B order",
    );
  });
  ...
  test("unaffected order (PDR-106) is on time in KEEP_CURRENT", () => {
    ...
    assert.equal(
      task!.status,
      "ON_TIME",
      "PDR-106 must be ON_TIME (unaffected)",
    );
  });
});
...
describe("16. financial invariants", () => {
  test("disruptedImpact - recoveryImpact = avoidedImpact (rounded)", () => {
    ...
    assert.ok(
      avoided > 0,
      `Avoided impact must be positive but got €${avoided}`,
    );
    assert.equal(
  ...
  test("baseline synthetic fixture: disrupted €6490, recovery €3400, avoided €3090", () => {
    ...
    assert.equal(
      disruptedTotal,
      6490,
      `Disrupted total expected €6490 but got €${disruptedTotal}`,
    );
    assert.equal(
      recoveryTotal,
      3400,
      `Recovery total expected €3400 but got €${recoveryTotal}`,
    );
...
describe("17. schedule and decision consistency", () => {
  ...
  test("MOVED orders in recovery plan have different lineId from KEEP_CURRENT", () => {
    ...
    assert.ok(
      movedOrders.length > 0,
      "Recovery plan must move at least one order to a different line",
    );
  });
  ...
  test("PDR-104 (CRITICAL, Machine B) is moved to a different line in recovery", () => {
    ...
    assert.notEqual(
      recTask!.lineId,
      keepTask!.lineId,
      "PDR-104 must be moved to a different line in recovery",
    );
  });
});
...
describe("18. sensitivity heading semantics", () => {
  ...
  test("sensitivity heading logic: hasBoundary is false when all entries feasible", () => {
    ...
    assert.equal(
      hasBoundary,
      false,
      "hasBoundary must be false for baseline config",
    );
  });
  ...
  test("sensitivity entries always have typed feasible and strategy fields regardless of config", () => {
    ...
    for (const entry of sensitivity) {
      assert.equal(
        typeof entry.feasible,
        "boolean",
        `entry.feasible must be boolean for ${entry.hours}h`,
      );
      assert.equal(
        typeof entry.strategy,
        "string",
        `entry.strategy must be string for ${entry.hours}h`,
      );
