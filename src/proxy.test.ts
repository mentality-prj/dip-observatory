...
test("routes localized marketing URLs and nested public pages",()=>{const home=proxy(request("qdip.ai","/uk"));const core=proxy(request("qdip.ai","/en/core/architecture"));const canonical=proxy(request("qdip.ai","/platform/pl/use-cases"));const preview=proxy(request("feature-qdip.vercel.app","/platform/uk"));const observatory=proxy(request("observatory.qdip.ai","/pl"));assert.equal(new URL(getRewrittenUrl(home)!).pathname,"/platform/uk");assert.equal(new URL(getRewrittenUrl(core)!).pathname,"/platform/en/core/architecture");assert.equal(canonical.status,308);assert.equal(new URL(canonical.headers.get("location")!).pathname,"/pl/use-cases");assert.equal(isRewrite(preview),false);assert.equal(isRewrite(observatory),false)});
