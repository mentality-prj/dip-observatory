...
import { resourceAllocationNetworkI18n } from '../i18n'
...
export function ResourceAllocationNetwork({
  ...
  const t = resourceAllocationNetworkI18n[locale]
      ...
      <div className="grid gap-2 px-4 py-4 md:hidden" aria-label={`${t.network}: ${day} mobile summary`}>
        {communities.map((community) => {
          ...
          return (
            <div
              key={community}
              className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 border-b border-white/10 py-3 last:border-b-0"
            >
              ...
              <span
                className={`rounded-full px-2 py-1 text-[10px] font-semibold ${movedTeams.length ? 'bg-rose-400/15 text-rose-300' : 'bg-white/5 text-white/45'}`}
              >
              ...
              <g key={community} transform={`translate(${point.x},${point.y})`}>
                <circle
                  ...
                  fill={
                    changed
                      ? mode === 'manager'
                        ? 'var(--ds-semantic-uncertainty)'
                        : 'var(--ds-semantic-selected)'
                      : '#242424'
                  }
                  stroke="white"
                  strokeOpacity="0.25"
