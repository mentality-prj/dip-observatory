...
export function SupplyNetworkResilienceWorkspace({ locale }: { locale: Locale }) {
  ...
  const bestBaselineLoss = result
    ? Math.min(...result.alternatives.map((item) => item.worst_case_business_loss))
    : 0
  const avoidedLoss = result
    ? Math.max(0, bestBaselineLoss - result.recommended.worst_case_business_loss)
    : 0
  const avoidedLossPct = bestBaselineLoss > 0 ? avoidedLoss / bestBaselineLoss : 0
  const worstScenario = result
    ? result.recommended.scenario_outcomes.reduce(
      (worst, item) => item.business_loss > worst.business_loss ? item : worst,
    )
    : null
  ...
  return <main className="mx-auto w-full max-w-[1540px] px-4 pb-20 sm:px-5 md:px-8 lg:px-10">
    <section className="grid gap-6 py-8 lg:grid-cols-[1.5fr_1fr] lg:items-end">
      <div><Badge variant="cyan">QDIP OBSERVATORY · SUPPLY NETWORK RESILIENCE</Badge><h1 className="ds-h1 mt-4 max-w-4xl">{t.title}</h1><p className="ds-lead mt-4 max-w-3xl text-slate-300">{t.question}</p></div>
      <Card><CardContent><div className="pt-6"><div className="grid grid-cols-3 gap-3 text-center"><div><Network className="mx-auto mb-2 h-5 w-5 text-[var(--ds-accent)]"/><strong>{SUPPLY_RESILIENCE_DEMO.nodes.length}</strong><p className="text-xs text-slate-500">{t.nodes}</p></div><div><ShieldCheck className="mx-auto mb-2 h-5 w-5 text-[var(--ds-accent)]"/><strong>{SUPPLY_RESILIENCE_DEMO.productClasses.length}</strong><p className="text-xs text-slate-500">{t.products}</p></div><div><AlertTriangle className="mx-auto mb-2 h-5 w-5 text-[var(--ds-accent)]"/><strong>{SUPPLY_RESILIENCE_DEMO.policy.planningHorizonDays}</strong><p className="text-xs text-slate-500">{t.horizon}</p></div></div><Button className="mt-6 w-full" onClick={run} disabled={running}><Play className="h-4 w-4" />{running ? t.running : t.run}</Button>{error ? <p className="mt-3 text-sm text-[var(--ds-semantic-danger)]">{error}</p> : null}</div></CardContent></Card>
    </section>

    {result ? <div className="space-y-8">
      <section>
        <div className="mb-3 flex items-center gap-3">
          <h2 className="ds-h2">{locale === 'uk' ? 'Що рекомендує QDIP' : locale === 'pl' ? 'Co rekomenduje QDIP' : 'What QDIP recommends'}</h2>
          <Badge variant={result.validation_gate.passes ? 'emerald' : 'amber'}>{result.validation_gate.passes ? t.validated : t.partial}</Badge>
        </div>
        <Card><CardContent className="pt-6">
          <div className="grid gap-4 md:grid-cols-3">
            <Metric label={locale === 'uk' ? 'Уникнені втрати' : locale === 'pl' ? 'Uniknięta strata' : 'Loss avoided'} value={money(avoidedLoss, locale)} />
            <Metric label={locale === 'uk' ? 'Зменшення втрат' : locale === 'pl' ? 'Redukcja strat' : 'Loss reduction'} value={pct(avoidedLossPct)} />
            <Metric label={locale === 'uk' ? 'Сервіс у найгіршому сценарії' : locale === 'pl' ? 'Obsługa w najgorszym scenariuszu' : 'Service in worst scenario'} value={worstScenario ? pct(worstScenario.service_level) : '—'} />
          </div>
          <p className="mt-5 text-sm text-slate-300">{locale === 'uk' ? 'Нижче — конкретні переміщення запасів і наслідки для кожного сценарію відмови.' : locale === 'pl' ? 'Poniżej: konkretne przesunięcia zapasów i skutki każdego scenariusza awarii.' : 'Below: concrete inventory moves and the outcome of each disruption scenario.'}</p>
        </CardContent></Card>
      </section>
      <section><div className="mb-3 flex items-center gap-3"><h2 className="ds-h2">{t.recommendation}</h2><Badge variant={result.validation_gate.passes ? 'emerald' : 'amber'}>{result.validation_gate.passes ? t.validated : t.partial}</Badge></div><p className="mb-5 text-sm text-slate-500">{result.evidence.method === 'robust-linear-minimax-optimization' ? (locale === 'uk' ? 'Оптимізація мінімізує втрати у найгіршому заданому сценарії.' : locale === 'pl' ? 'Optymalizacja minimalizuje straty w najgorszym zadanym scenariuszu.' : 'Optimization minimizes loss in the worst specified scenario.') : t.heuristic} Engine {result.evidence.engine_version}.</p><AlternativeCard item={result.recommended} locale={locale} label={t.recommendation} /></section>
