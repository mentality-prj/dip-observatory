...
export function SupplyNetworkResilienceWorkspace({ locale }: { locale: Locale }) {
  ...
  return <main className="mx-auto w-full max-w-[1540px] px-4 pb-20 sm:px-5 md:px-8 lg:px-10">
    <section className="grid gap-6 py-8 lg:grid-cols-[1.5fr_1fr] lg:items-end">
      ...
      <Card><CardContent className="pt-12"><div className="grid grid-cols-3 gap-3 text-center"><div><Network className="mx-auto mb-2 h-5 w-5 text-[var(--ds-accent)]"/><strong>{SUPPLY_RESILIENCE_DEMO.nodes.length}</strong><p className="text-xs text-slate-500">{t.nodes}</p></div><div><ShieldCheck className="mx-auto mb-2 h-5 w-5 text-[var(--ds-accent)]"/><strong>{SUPPLY_RESILIENCE_DEMO.productClasses.length}</strong><p className="text-xs text-slate-500">{t.products}</p></div><div><AlertTriangle className="mx-auto mb-2 h-5 w-5 text-[var(--ds-accent)]"/><strong>{SUPPLY_RESILIENCE_DEMO.policy.planningHorizonDays}</strong><p className="text-xs text-slate-500">{t.horizon}</p></div></div><Button className="mt-6 w-full" onClick={run} disabled={running}><Play className="h-4 w-4" />{running ? t.running : t.run}</Button>{error ? <p className="mt-3 text-sm text-[var(--ds-semantic-danger)]">{error}</p> : null}</CardContent></Card>
    </section>
