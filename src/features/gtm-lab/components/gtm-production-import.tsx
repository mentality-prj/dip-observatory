...
export function GtmProductionImport({ onEvaluated }: Props) {
  ...
  async function selectFile(file: File) {
    ...
    try {
      ...
      setIssues(
        validation.issues.map((issue) => `Row ${issue.row}${issue.field ? ` · ${issue.field}` : ''}: ${issue.message}`)
      )
      setMessage(
        validation.valid
          ? `${validation.valid_rows} prospects validated. Ready to run QDIP.`
          : `${validation.invalid_rows} rows require correction.`
      )
    } catch (error) {
  ...
  async function runPipeline() {
    ...
    try {
      ...
      onEvaluated(result)
      setMessage(
        `${result.summary.evaluated_prospects}/${result.summary.total_prospects} companies evaluated${result.summary.failed_prospects ? ` · ${result.summary.failed_prospects} require correction` : ''}.`
      )
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'QDIP pipeline failed.')
    } finally {
      setRunning(false)
    }
  }
    ...
    <section className="mt-6 border border-white/10 bg-white/[.03] p-5" aria-labelledby="gtm-production-import">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-slate-400">
            Upload CSV data. QDIP validates the canonical contract and remains the decision authority.
          </p>
        </div>
      ...
      {rows.length > 0 && !issues.length && (
        <button
          ...
          disabled={running}
          className="mt-4 bg-sky-400 px-5 py-2.5 font-bold text-slate-950 disabled:opacity-50"
        >
          {running ? (
            <>
            ...
            `Run evaluation for ${rows.length} companies`
          )}
        </button>
      )}
    </section>
