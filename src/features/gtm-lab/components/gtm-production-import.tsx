...
import { importValidationSchema, pipelineRunSchema, type PipelineRun, type ProspectSeed } from "../import-contracts";

type Props = { onEvaluated: (run: PipelineRun) => void };
...
export function GtmProductionImport({ onEvaluated }: Props) {
  ...
  async function runPipeline() {
    ...
    try {
      ...
      onEvaluated(result);
      setMessage(`${result.summary.evaluated_prospects}/${result.summary.total_prospects} companies evaluated${result.summary.failed_prospects ? ` · ${result.summary.failed_prospects} require correction` : ""}.`);
  ...
  return <section className="mt-6 border border-white/10 bg-white/[.03] p-5" aria-labelledby="gtm-production-import">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><span className="text-[10px] font-black tracking-[.18em] text-sky-300">IMPORTED DATA</span><h2 id="gtm-production-import" className="mt-1 text-lg font-black">Evaluate your prospect portfolio</h2><p className="mt-1 max-w-2xl text-sm text-slate-400">Upload CSV data. QDIP validates the canonical contract and remains the decision authority.</p></div><label className="cursor-pointer border border-white/15 px-4 py-2 text-sm font-bold hover:bg-white/[.05]"><FileUp className="mr-2 inline h-4 w-4"/>Upload CSV<input className="sr-only" type="file" accept=".csv,text/csv" onChange={(event) => { const file = event.target.files?.[0]; if (file) void selectFile(file); event.currentTarget.value = ""; }}/></label></div>
    {message && <p className="mt-4 text-sm text-slate-300" aria-live="polite">{message}</p>}
    {issues.length > 0 && <div className="mt-4 border border-amber-400/20 p-3 text-sm text-amber-200"><CircleAlert className="mr-2 inline h-4 w-4"/><b>Validation errors</b><ul className="mt-2 space-y-1">{issues.slice(0, 20).map((issue) => <li key={issue}>{issue}</li>)}</ul></div>}
    {rows.length > 0 && !issues.length && <button type="button" onClick={() => void runPipeline()} disabled={running} className="mt-4 bg-sky-400 px-5 py-2.5 font-bold text-slate-950 disabled:opacity-50">{running ? <><LoaderCircle className="mr-2 inline h-4 w-4 animate-spin"/>Evaluating…</> : `Run evaluation for ${rows.length} companies`}</button>}
  </section>;
}
