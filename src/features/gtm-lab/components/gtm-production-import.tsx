...
export function GtmProductionImport({ locale, onEvaluated }: Props) {
  ...
  async function selectFile(file: File) {
    ...
    try {
      ...
      setIssues(
        validation.issues.map((issue) => t.issue(issue.row, issue.field, issue.message))
      )
      setMessage(
        validation.valid
          ? t.validated(validation.valid_rows)
          : t.invalidRows(validation.invalid_rows)
      )
    } catch (error) {
  ...
  async function runEvaluation() {
    ...
    try {
      ...
      setMessage(
        t.completed(
          result.summary.evaluated_prospects,
          result.summary.total_prospects,
          result.summary.failed_prospects
        )
      )
    } catch {
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <span className="text-[10px] font-semibold tracking-[.18em] text-sky-300">
            {t.tag}
          </span>
