...
export function GtmCommercialContext({ locale, value, onChange }: Props) {
  ...
  return (
    <section className="mt-7 border border-sky-400/20 bg-sky-400/[.04] p-5 md:p-7" aria-labelledby="gtm-context-title">
      ...
      <h2 id="gtm-context-title" className="mt-2 text-2xl font-medium">{t.title}</h2>
      ...
      <form onSubmit={submit} className="mt-6 grid gap-4 md:grid-cols-2">
        {fields.map(([label, fieldValue, setter, placeholder], index) => (
          <label key={label} className={index < 2 ? 'md:col-span-2' : ''}>
            ...
            {index < 2 ? (
              <textarea value={fieldValue} onChange={(event) => setter(event.target.value)} placeholder={placeholder} rows={2} className="w-full border border-white/15 bg-slate-950/70 p-3 text-sm" />
            ) : (
              <input value={fieldValue} onChange={(event) => setter(event.target.value)} placeholder={placeholder} className="w-full border border-white/15 bg-slate-950/70 p-3 text-sm" />
            )}
          </label>
        ))}
        <div className="md:col-span-2">
          <button type="submit" className="bg-sky-400 px-5 py-2.5 font-bold text-slate-950">{t.continue}</button>
