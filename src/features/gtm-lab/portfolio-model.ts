...
export function fromDemo(data: GtmDemo): PortfolioModel {
  return {
    ...
    items: data.prospects.map((prospect) => ({
      ...
      confidence: prospect.decision.confidence ?? null,
      ...
      evidence: prospect.evidence.map((evidence) => ({
        ...
        observedAt: evidence.observed_at ?? evidence.collected_at ?? undefined,
...
function portfolioItemFromPipeline(prospect: PipelineProspect, opportunityId: string): PortfolioItem | null {
  ...
  return {
    id: decisionIdentity(prospect.company.id, opportunity.id, decision.provenance?.decision_id),
    companyId: prospect.company.id,
    ...
    reasons: details?.reasons ?? decision.explanation,
    risks: details?.risks ?? [],
    ...
    capability: capability
      ? {
          ...
          problem: capability.problem_statement,
          rationale: capability.rationale,
        }
      : null,
    nextAction: decision.next_action
      ? {
          type: decision.next_action.type,
          title: decision.next_action.title,
          description: decision.next_action.description,
          targetRole: decision.next_action.target_role,
