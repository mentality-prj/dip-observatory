...
function routeCapacityIsActuallyBinding(label: string, result: OptimizationResult, network: SupplyNetwork) {
  ...
  if (kind === 'delivery-route-capacity') {
    const route = network.delivery_routes.find((item) => item.from_node_id === from && item.to_demand_point_id === to)
    if (!route) return false
    const used = result.fulfillment
      .filter((item) => item.warehouse_id === from && item.demand_point_id === to && item.departure_period === period)
  ...
  if (kind === 'transfer-route-capacity') {
    const route = network.transfer_routes.find((item) => item.from_node_id === from && item.to_node_id === to)
    if (!route) return false
    const used = result.transfers
      .filter(
        (item) => item.from_warehouse_id === from && item.to_warehouse_id === to && item.departure_period === period
      )
      .reduce((sum, item) => sum + item.units, 0)
    const tolerance = Math.max(1, route.capacity_units_per_day) * 1e-5
    return Math.abs(used - route.capacity_units_per_day) <= tolerance
