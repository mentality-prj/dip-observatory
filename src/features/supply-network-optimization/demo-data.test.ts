    ...
    for (const point of SUPPLY_NETWORK_DEMO.demand_points) {
      for (const product of SUPPLY_NETWORK_DEMO.product_classes) {
        const allocated = SUPPLY_NETWORK_DEMO.baseline_fulfillment
          .filter((item) => item.demand_point_id === point.id && item.product_class_id === product.id)
...
it('never assigns a baseline product to an incompatible warehouse', () => {
  const productById = new Map(SUPPLY_NETWORK_DEMO.product_classes.map((product) => [product.id, product]))
  const warehouseById = new Map(SUPPLY_NETWORK_DEMO.warehouses.map((warehouse) => [warehouse.id, warehouse]))
