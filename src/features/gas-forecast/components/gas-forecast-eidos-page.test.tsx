...
test("renders success execution state when result is present", () => {
  const html = renderToStaticMarkup(<GasForecastEidosPage initialResult={{ status: "succeeded", httpStatus: 200, responseTimeMs: 123, message: null, payload: { execution_id: "exp-1" }, executedAt: "2026-01-01T00:00:00.000Z" }} />);
...
test("renders failure message when execution fails", () => {
  const html = renderToStaticMarkup(<GasForecastEidosPage initialResult={{ status: "failed", httpStatus: 500, responseTimeMs: 321, message: "Provider timeout", payload: { detail: "Provider timeout" }, executedAt: "2026-01-01T00:00:00.000Z" }} />);
...
test("renders null payload safely for failure responses", () => {
  const html = renderToStaticMarkup(<GasForecastEidosPage initialResult={{ status: "failed", httpStatus: 503, responseTimeMs: null, message: "Not configured", payload: null, executedAt: "2026-01-01T00:00:00.000Z" }} />);
