...
import { ENTSOG_POINT_PRESETS, type EntsogPointPreset } from '@/lib/entsog-point-directory'
...
import { WEATHER_REGION_PRESETS, type WeatherRegionPreset } from '@/lib/weather-region-presets'
  ...
  return (
    <div className="space-y-4 rounded-[20px] border border-white/8 bg-black/20 p-4">
      <div className="space-y-1 text-sm text-slate-200">
        ...
        <p>
          <span className="text-slate-500">Failure type:</span> {result.kind ?? '—'}
        </p>
        <p>
      ...
      {result.dataset ? (
        <div className="space-y-2 text-sm text-slate-200">
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-cyan-400">Dataset</p>
          <p>
            <span className="text-slate-500">records:</span> {result.dataset.records ?? '—'}
          </p>
          <p>
            <span className="text-slate-500">first date:</span> {result.dataset.firstDate ?? '—'}
          </p>
          <p>
            <span className="text-slate-500">last date:</span> {result.dataset.lastDate ?? '—'}
          </p>
          <p>
            <span className="text-slate-500">country/facility:</span> {result.dataset.countryOrFacility ?? '—'}
          </p>
        </div>
      ) : null}
      ...
      {result.message ? (
        <div className="rounded-2xl border border-rose-300/20 bg-rose-300/8 p-3 text-sm text-rose-100">
          ...
          {result.errorDetail?.code ? (
            <p>
              <span className="text-rose-300/70">Code:</span> {result.errorDetail.code}
            </p>
          ) : null}
          {result.errorDetail?.upstreamStatus !== null && result.errorDetail?.upstreamStatus !== undefined ? (
            <p>
              <span className="text-rose-300/70">Upstream status:</span> {result.errorDetail.upstreamStatus}
            </p>
          ) : null}
          {result.errorDetail?.executionId ? (
            <p>
              <span className="text-rose-300/70">Execution id:</span> {result.errorDetail.executionId}
            </p>
          ) : null}
        </div>
      ) : null}
              ...
              <table className="min-w-full border-collapse text-left text-xs text-slate-200">
                ...
                <tbody>
                  {result.sample.map((row, index) => (
                    ...
                    >
                      <td className="px-3 py-2">{(row.observation_date as string | undefined) ?? '—'}</td>
                      <td className="px-3 py-2">{(row.settlement_price as string | number | undefined) ?? '—'}</td>
                      <td className="px-3 py-2">{(row.source as string | undefined) ?? '—'}</td>
                      <td className="px-3 py-2">{(row.source_identifier as string | undefined) ?? '—'}</td>
                      <td className="px-3 py-2">{(row.publication_date as string | undefined) ?? '—'}</td>
                      <td className="px-3 py-2">{(row.retrieved_at as string | undefined) ?? '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {providerId === 'ttf' && result.connection === 'OK' ? (
        <div className="space-y-1 rounded-2xl border border-emerald-300/20 bg-emerald-300/8 p-3 text-sm text-emerald-100">
          ...
          <p>
            <span className="text-emerald-300/80">Records:</span> {result.dataset?.records ?? '—'}
          </p>
...
export function buildTtfCheckInput(config: GasForecastTtfCheckInput): GasForecastTtfCheckInput {
  return {
...
export function validateTtfCheckInput(config: GasForecastTtfCheckInput, todayIso: string) {
...
export function FlowPointCombobox({
  ...
  const options = useMemo(() => {
    ...
    return presets.filter((preset) => preset.label.toLowerCase().includes(normalizedQuery))
  }, [presets, query])
  ...
  return (
    <div className="min-w-0 w-full space-y-2" ref={containerRef}>
      ...
      {presets.length === 0 ? <p className="text-xs text-slate-400">No ENTSOG flow points available.</p> : null}
    ...
    <div className="min-w-0 w-full space-y-2" ref={containerRef}>
      ...
      <div className="relative min-w-0 w-full">
        ...
        >
            ...
            onKeyDown={(event) => {
              ...
              if (event.key === 'ArrowDown') {
                ...
                setHighlightedIndex((current) =>
                  options.length === 0 ? -1 : Math.min(Math.max(current + 1, 0), options.length - 1)
                )
                return
              }
              ...
              if (event.key === 'ArrowUp') {
                event.preventDefault()
                setHighlightedIndex((current) => (options.length === 0 ? -1 : current <= 0 ? 0 : current - 1))
                return
              }
          ...
          />
          <ChevronDown className="pointer-events-none ml-auto h-4 w-4 shrink-0 text-slate-400" aria-hidden="true" />
        </div>
          ...
          >
            {options.length > 0 ? (
              options.map((option) => {
                ...
                const highlighted = option.value === options[highlightedIndex]?.value
...
export function GasForecastProvidersPage() {
  const [results, setResults] = useState<Partial<Record<GasForecastProviderId, GasForecastConnectionResult>>>({})
  const [pendingProviderId, setPendingProviderId] = useState<GasForecastProviderId | null>(null)
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
        <header className="space-y-3">
          ...
          <div className="space-y-1">
            <h1 className="text-2xl font-semibold tracking-tight text-white md:text-3xl">Gas Forecast Providers</h1>
            <p className="max-w-3xl text-sm text-slate-400">
              Test the real DIP → PluginRuntime → provider connectivity path. No browser-side provider credentials are
              used.
            </p>
          </div>
        </header>
        ...
        <section className="grid gap-4 md:grid-cols-2">
          {GAS_FORECAST_PROVIDER_CARDS.map((provider) => {
            ...
            const currentStatus = getCardStatus(provider, result, pendingProviderId)
                ...
                <CardHeader className="space-y-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    >
                      {currentStatus === 'connected' ? (
                        ...
                        <Activity
                          className={cn('h-5 w-5', currentStatus === 'testing' ? 'animate-pulse' : '')}
                          aria-hidden="true"
                        />
                      )}
                    </div>
                  ...
                  <div className="space-y-1 text-sm text-slate-300">
                    {provider.id === 'ttf' ? (
                      <>
                        <p>
                          <span className="text-slate-500">Description:</span> {provider.description}
                        </p>
                        <p>
                        ...
                        <p>
                          <span className="text-slate-500">Frequency:</span> {provider.frequency}
                        </p>
                      </>
                    ) : null}
                    {provider.api ? (
                    ...
                    {result?.httpStatus !== null && result?.httpStatus !== undefined ? (
                      <p>
                        <span className="text-slate-500">Last response:</span> {result.httpStatus}
                      </p>
                    ) : null}
                    {result?.dataset?.records !== null && result?.dataset?.records !== undefined ? (
                      <p>
                        <span className="text-slate-500">Records:</span> {result.dataset.records}
                      </p>
                    ) : null}
                ...
                <CardContent className="min-w-0 w-full max-w-full space-y-4">
                  ...
                  {provider.id === 'entsog' ? (
                    <div className="min-w-0 w-full max-w-full space-y-3 rounded-[20px] border border-white/8 bg-black/20 p-4">
                      <p className="text-xs font-semibold uppercase tracking-[0.22em] text-cyan-400">ENTSOG query</p>
                      <FlowPointCombobox
                          ...
                          <Input
                            ...
                            onChange={(event) => {
                              ...
                              setEntsogConfig((current) => ({
                                ...
                                to: current.to && event.target.value > current.to ? event.target.value : current.to,
                              }))
                            }}
                          />
                        </div>
                      ...
                      {entsogValidationError ? <p className="text-xs text-rose-300">{entsogValidationError}</p> : null}
                  ...
                  {provider.id === 'weather' ? (
                    <div className="min-w-0 w-full max-w-full space-y-3 rounded-[20px] border border-white/8 bg-black/20 p-4">
                      <p className="text-xs font-semibold uppercase tracking-[0.22em] text-cyan-400">Weather query</p>
                  ...
                  {provider.id === 'ttf' ? (
                    <div className="min-w-0 w-full max-w-full space-y-3 rounded-[20px] border border-white/8 bg-black/20 p-4">
                      <p className="text-xs font-semibold uppercase tracking-[0.22em] text-cyan-400">TTF query</p>
                      <div className="grid w-full min-w-0 max-w-full gap-3 md:grid-cols-2">
                        <div className="min-w-0 space-y-2">
