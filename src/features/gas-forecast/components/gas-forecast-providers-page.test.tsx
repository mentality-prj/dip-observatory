...
test('renders static ENTSOG presets into the combobox selection flow', () => {
  ...
  const html = renderToStaticMarkup(
    <FlowPointCombobox presets={ENTSOG_POINT_PRESETS} selectedValue={selectedValue} onSelect={() => {}} initialOpen />
  )
...
test('validates and trims TTF query input before submission', () => {
  assert.equal(
    validateTtfCheckInput({ start_date: '2999-01-01', end_date: '2999-01-02' }, '2026-01-07'),
    'Future dates are not available.'
  )
  assert.deepEqual(buildTtfCheckInput({ start_date: ' 2026-01-01 ', end_date: ' 2026-01-07 ' }), {
