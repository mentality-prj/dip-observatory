...
export function GasForecastEidosPage({ initialResult = null }: { initialResult?: GasForecastExperimentResult | null }) {
  ...
  function exportRawPayload() {
    if (!result) return
    const blob = new Blob([JSON.stringify(result.payload, null, 2) ?? ''], { type: 'text/plain;charset=utf-8' })
    ...
    <main className="relative min-h-screen overflow-hidden px-4 py-6 md:px-6 xl:px-10">
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-6">
        <header className="space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold uppercase tracking-[0.22em] text-cyan-400">QDIP Observatory</span>
              <Badge variant="amber" className="gap-1.5"><FlaskConical className="h-3 w-3" aria-hidden="true" />Research prototype</Badge>
            </div>
          ...
          <div className="space-y-1">
            <h1 className="text-2xl font-semibold tracking-tight text-white md:text-3xl">Gas Procurement Decision</h1>
            <p className="max-w-3xl text-sm text-slate-400">Evaluate how required gas volume could be secured under uncertainty. Commercial value remains unvalidated until the frozen-dataset economic gate passes.</p>
          </div>
        </header>

        <GasProcurementDecisionDemo volumeMwh={form.volume_mwh} deadline={form.end_date} horizonDays={form.forecast_horizon_days} />

        <Card><CardHeader><CardTitle>Research scenario</CardTitle></CardHeader><CardContent className="space-y-4">
          <form className="space-y-4" onSubmit={(event) => {
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-2"><Label htmlFor="start-date">Historical start</Label><Input id="start-date" type="date" className="h-11 min-h-11 max-h-11 appearance-none" value={form.start_date} onChange={(event) => update('start_date', event.target.value)} /></div>
              <div className="space-y-2"><Label htmlFor="end-date">Procurement deadline</Label><Input id="end-date" type="date" className="h-11 min-h-11 max-h-11 appearance-none" value={form.end_date} onChange={(event) => update('end_date', event.target.value)} /></div>
              <div className="space-y-2"><Label htmlFor="forecast-horizon-days">Decision horizon, days</Label><Input id="forecast-horizon-days" type="number" min={1} step={1} value={String(form.forecast_horizon_days)} onChange={(event) => update('forecast_horizon_days', Number(event.target.value || '0'))} /></div>
              <div className="space-y-2"><Label htmlFor="volume-mwh">Required volume, MWh</Label><Input id="volume-mwh" type="number" min={1} step={1} value={String(form.volume_mwh)} onChange={(event) => update('volume_mwh', Number(event.target.value || '0'))} /></div>
              <div className="space-y-2 md:col-span-2"><Label htmlFor="procurement-threshold">Legacy research threshold, €/MWh</Label><Input id="procurement-threshold" type="number" step={0.01} value={String(form.procurement_threshold_eur_per_mwh)} onChange={(event) => update('procurement_threshold_eur_per_mwh', Number(event.target.value || '0'))} /></div>
            </div>
            <Button type="submit" size="lg" className="w-full" disabled={isPending}>{isPending ? 'Running research evaluation...' : 'Run research evaluation'}</Button>
          </form>
        </CardContent></Card>
        ...
        {result ? <Card><CardHeader className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3"><CardTitle>Technical execution</CardTitle><Badge variant={result.status === 'succeeded' ? 'emerald' : 'rose'}>{result.status === 'succeeded' ? 'SUCCEEDED' : 'FAILED'}</Badge></div>
