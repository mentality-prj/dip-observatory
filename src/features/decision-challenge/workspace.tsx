...
function newSubmissionId() {
  return typeof crypto !== 'undefined' && 'randomUUID' in crypto ? crypto.randomUUID() : `submission-${Date.now()}`
}
...
export function DecisionChallengeWorkspace({ locale }: { locale: Locale }) {
  ...
  const comparisonConsistent =
    result != null &&
    result.human_economic_outcome.objective.objective_id === result.qdip_economic_outcome.objective.objective_id &&
    result.human_economic_outcome.objective.metric_id === result.qdip_economic_outcome.objective.metric_id &&
    result.human_economic_outcome.objective.direction === result.qdip_economic_outcome.objective.direction &&
    result.human_economic_outcome.objective.unit === result.qdip_economic_outcome.objective.unit
  const verdict =
  ...
  return (
    <main className="ds-container ds-page">
            ...
            <label className="ds-button ds-button-primary ds-button-md cursor-pointer">
              ...
              <input
                ...
                onChange={(event) => {
                  ...
                  void importCustomerScenario(file)
          ...
          <div>
            ...
            <div className="grid gap-3">
              {scenario.teams.map((team) => (
                <div
                  key={team.id}
                  className="rounded-[var(--ds-radius-panel)] border border-white/10 bg-white/[0.03] p-4"
                >
      ...
      <section className="ds-card mt-6 p-6 sm:p-8">
        ...
        {violations.length > 0 && (
          <div className="mt-5 rounded-[var(--ds-radius-panel)] border border-rose-300/20 bg-rose-300/[0.06] p-4 text-sm text-rose-100">
            {violations.map((item) => (
...
function ValueCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[var(--ds-radius-panel)] border border-white/10 bg-white/[0.03] p-5">
  ...
  return (
    <div className="rounded-[var(--ds-radius-panel)] border border-white/10 bg-white/[0.03] p-5">
      ...
      <div className="mt-4 grid gap-2">
        {Object.entries(allocation).map(([team, target]) => (
          <div key={team} className="flex justify-between gap-4 border-t border-white/5 pt-2 text-sm">
...
function Explanation({ item }: { item: Record<string, unknown> }) {
  ...
  return (
    <div className="rounded-[var(--ds-radius-panel)] border border-white/10 bg-white/[0.03] p-4">
...
function Meta({ label, value }: { label: string; value: string }) {
  return (
    <div className="grid grid-cols-[120px_1fr] gap-4 border-t border-white/5 pt-3">
