    ...
    <main className="ds-container ds-page">
      ...
      <section className="ds-card mb-6 p-6 sm:p-8">
        ...
        <ol className="mt-5 grid gap-3 text-sm leading-6 text-slate-400">
          {howSteps.map((step, index) => <li key={step} className="flex gap-3"><span className="font-mono text-cyan-300">{index + 1}.</span><span>{step}</span></li>)}
        </ol>
        ...
        <>
          <section className="ds-card mt-6 p-6 sm:p-8" data-testid="challenge-comparison">
            ...
            {verdict ? (
              ...
              <p role="alert" className="mt-3 text-sm text-rose-200">{t('comparisonUnavailable')}</p>
            )}
          ...
          <section className="mt-6 grid gap-6 lg:grid-cols-2">
            ...
            <div className="ds-card p-6 sm:p-8">
              ...
              <dl className="mt-5 grid gap-3 text-sm">
                ...
                <Meta label={t('evaluator')} value={`${result.evaluation_reproducibility.evaluator_id} · ${result.evaluation_reproducibility.evaluator_version}`} />
                <Meta label={t('evaluationContext')} value={result.evaluation_reproducibility.context_hash.slice(0, 16)} />
              </dl>
            </div>
          </section>
          ...
          <section className="mt-6 rounded-[var(--ds-radius-panel)] border border-cyan-300/20 bg-cyan-300/[0.06] p-6 sm:p-8">
            ...
            <a href={`${marketingHref(locale)}#decision`} onClick={() => void recordChallengeCta(run.id)} className="ds-button ds-button-primary ds-button-lg mt-5">{t('cta')}<ArrowRight className="h-4 w-4" /></a>
          </section>
        </>
      )}
    </main>
...
function Explanation({ item }: { item: Record<string, unknown> }) {
  ...
  return <div className="rounded-[var(--ds-radius-panel)] border border-white/10 bg-white/[0.03] p-4"><div className="font-semibold">{team}{target ? ` → ${target}` : ''}</div><div className="mt-2 text-xs leading-5 text-slate-400">{services}{services && priority != null ? ' · ' : ''}{priority != null ? `${t('priorityDemand')}: ${priority}` : ''}</div></div>
}
