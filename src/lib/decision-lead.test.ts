...
test("Zoho delivery uses SMTP mailbox as sender and recipient without sending real email",async()=>{const old={...process.env};Object.assign(process.env,{ZOHO_SMTP_USER:"hello@qdip.ai",ZOHO_SMTP_PASSWORD:"app-password"});let captured="";const transport:SmtpTransport={send:async message=>{captured=message}};try{await new ZohoMailDelivery(transport).sendDecisionLead(decisionLeadSchema.parse(valid),"submission-1");assert.match(captured,/From: QDIP <hello@qdip\.ai>/);assert.match(captured,/To: hello@qdip\.ai/);assert.match(captured,/Reply-To: ada@example\.org/);assert.match(captured,/QDIP decision inquiry/)}finally{process.env=old}});
test("Zoho delivery failure is surfaced without sending a real email",async()=>{const old={...process.env};Object.assign(process.env,{ZOHO_SMTP_USER:"hello@qdip.ai",ZOHO_SMTP_PASSWORD:"app-password"});const transport:SmtpTransport={send:async()=>{throw new Error("smtp_delivery_failed")}};try{await assert.rejects(()=>new ZohoMailDelivery(transport).sendDecisionLead(decisionLeadSchema.parse(valid),"submission-2"),/smtp_delivery_failed/)}finally{process.env=old}});
