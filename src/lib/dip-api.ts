...
function getDipApiKey() { return (process.env.DIP_API_KEY ?? process.env.DIP_ADMIN_API_KEY ?? "").trim(); }
function buildDipUrl(path: string) { return `${getDipBaseUrl()}${path.startsWith("/") ? path : `/${path}`}`; }
...
async function dipFetch<T>(path: string, schema: z.ZodType<T>, init?: RequestInit) {
  ...
  if (!baseUrl || !apiKey) throw new DipApiError("DIP API is not configured. Set DIP_API_BASE_URL and DIP_API_KEY.", 503);
...
export async function runDipPlugin(pluginName: string, capabilityId: string, input: Record<string, unknown>): Promise<Record<string, unknown>> {
  const response = await dipFetch(
    ...
    {
      method: "POST",
      body: JSON.stringify({ capability_id: capabilityId, input, config: {}, metadata: { source: "dip-observatory" }, features: input }),
    },
  );
