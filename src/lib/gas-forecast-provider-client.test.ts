...
import { getGasForecastCapabilityPaths, testGasForecastProviderConnection } from '@/lib/gas-forecast-provider-client'
...
test('returns a clear invalid-endpoint message after all fallback paths return 404', async () => {
  ...
  assert.equal(
    getGasForecastCapabilityPaths().every((path) => (result.message ?? '').includes(path)),
    true
  )
})
...
test('classifies a non-2xx, non-auth DIP HTTP response as kind=dip_http', async () => {
  ...
  globalThis.fetch = (async () => Response.json({ detail: 'Internal server error' }, { status: 500 })) as typeof fetch
...
test("never surfaces the request's DIP_API_KEY in the failure message even if a non-JSON DIP body echoes it back", async () => {
  ...
  globalThis.fetch = (async () =>
    new Response('Internal Server Error: header x-api-key: super-secret-key rejected by upstream', {
      status: 500,
