...
const alternatives={
 en:{label:"Alternatives",items:["Option A","Option B","Option C"],rejected:"constraint",recommended:"recommended",why:"why?"},
 uk:{label:"Альтернативи",items:["Варіант A","Варіант B","Варіант C"],rejected:"обмеження",recommended:"рекомендовано",why:"чому?"},
 pl:{label:"Alternatywy",items:["Opcja A","Opcja B","Opcja C"],rejected:"ograniczenie",recommended:"rekomendowana",why:"dlaczego?"},
} as const;
...
export function SignatureDecisionVisual({labels,locale}:{labels:readonly string[];locale:MarketingLocale}){
 ...
 return <figure className={styles.visual} aria-label={labels.join(" → ")}>
  ...
  <ArrowRight className={styles.arrow} aria-hidden="true" size={20}/>
  <div className={styles.evaluation}><small>{a.label}</small>{a.items.map((item,index)=><span className={index===1?styles.selected:index===2?styles.rejected:undefined} key={item}>{index===1&&<Check size={12}/>} {index===2&&<X size={12}/>} {item}{index===1&&<em>{a.recommended}</em>}{index===2&&<em>{a.rejected}</em>}</span>)}</div>
  <ArrowRight className={styles.arrow} aria-hidden="true" size={20}/>
  <div className={styles.result}><strong>{labels[5]}</strong><span>{labels[6]}</span><em>{a.why}</em></div>
  <ArrowRight className={styles.arrow} aria-hidden="true" size={20}/>
