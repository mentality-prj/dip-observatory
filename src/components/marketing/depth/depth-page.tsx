...
const localeHref=(l:MarketingLocale,path:string)=>`/${l}/${path}`;
const demoHref=(l:MarketingLocale,path:string)=>`/${l}${path}`;
...
export function DepthPage({locale,page}:{locale:MarketingLocale;page:DepthPageKey}){
 ...
 return <main className={styles.page}>
  <header className={styles.header}><Link href={`/${locale}`} aria-label="QDIP home"><QdipLogo/></Link><nav className={styles.primaryNav}>{c.nav.map((x,i)=><Link key={x} className={paths[i]===page?styles.active:undefined} href={localeHref(locale,paths[i])}>{x}</Link>)}</nav><nav aria-label="Language" className={styles.languageNav}>{marketingLocales.map(l=><Link aria-current={locale===l?"page":undefined} href={localeHref(l,page)} key={l}>{localeLabels[l]}</Link>)}</nav><Link className={styles.demo} href={localeHref(locale,"use-cases#demos")}>Demos <ArrowRight size={14}/></Link></header>
