      ...
      <main id="main-content" lang={locale}>
        ...
        <section className={styles.hero}>
          ...
          <div className={styles.heroActions}>
            <Link className={styles.primary} href={localeHref(locale, 'use-cases#demos')}>Explore working demos <ArrowRight size={15} /></Link>
            <Link className={styles.secondary} href={localeHref(locale, 'decision')}>Describe your decision</Link>
          </div>
        </section>
        {c.cases && <section id="demos" className={styles.cases}>{c.cases.map((x) => <article key={x.title} className={styles.case}><div className={styles.pattern}>{x.pattern}</div><h2>{x.title}</h2><h3>{x.question}</h3><p>{x.problem}</p><dl><div><dt>INPUTS</dt><dd>{x.inputs}</dd></div><div><dt>EVALUATION</dt><dd>{x.evaluation}</dd></div><div><dt>OUTPUT</dt><dd>{x.output}</dd></div></dl><Link href={observatoryHref(x.href, locale)}>Open {x.title} <ArrowRight size={14} /></Link></article>)}</section>}
