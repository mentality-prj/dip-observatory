...
export function QdipSite({locale="en"}:{locale?:MarketingLocale}){
  ...
  return <main className={styles.site} id="main-content" lang={locale}><Header locale={locale}/>
    <section className={styles.hero}><div className={styles.heroCopy}><div className={styles.eyebrow}>{c.hero[0]}</div><h1>{c.hero[1]}</h1><p className={styles.lead}>{c.hero[2]}</p><p>{c.hero[3]}</p><strong className={styles.human}>{c.hero[4]}</strong><div className={styles.heroActions}><Link className={styles.primaryButtonLarge} href={path(locale,"use-cases")}><Play size={16}/>{c.hero[5]}</Link></div></div><SignatureDecisionVisual labels={c.diagram}/></section>
