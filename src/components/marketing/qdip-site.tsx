...
export function QdipSite({locale="en"}:{locale?:MarketingLocale}){
  ...
  return <main className={styles.site} id="main-content" lang={locale}><Header locale={locale}/>
    ...
    <section className={styles.softSection} id="why"><div className={styles.sectionIntro}><h2>{c.why[0]}</h2></div><div className={styles.valueGrid}>{valueIndexes.map((i,index)=><article className={index<3?styles.primaryValue:styles.supportingValue} key={c.why[i]}><Check size={18}/><h3>{c.why[i]}</h3><p>{c.why[i+1]}</p></article>)}</div></section>
