...
export function MobileMenu({ items }: { items: readonly Item[] }) {
  ...
  return <div className={styles.mobileMenu}>
    ...
    {open && <nav aria-label="Mobile navigation" className={styles.mobileNav} id="mobile-navigation">
      {items.map(item => <Link href={item.href} key={`${item.href}-${item.label}`} onClick={() => setOpen(false)}>{item.label}</Link>)}
    </nav>}
  </div>;
}
