...
export function PrototypeShell({ locale, children, theme = "cyan" }: PrototypeShellProps) {
  ...
  const nav=<nav className={styles.navigation} aria-label="Observatory applications">{navItems.map(item=>{const active=isActive(item.route);return <Link ref={active?activeNavRef:undefined} key={item.id} href={buildLocalePath(item.route,locale)} aria-current={active?"page":undefined} className={styles.navLink}>{item.title[locale]}</Link>})}</nav>;
  const utilities=<div className={styles.locale} aria-label="Language">{LOCALES.map(option=><button key={option} type="button" disabled={pending} onClick={()=>changeLocale(option)} aria-current={option===locale?"page":undefined}>{LABEL[option]}</button>)}</div>;
  return <ProductShell theme={theme as DesignTheme} className={styles.shell} href={buildLocalePath("/",locale)} product="Observatory" navigation={<div className={styles.desktopNavigation}>{nav}</div>} mobileNavigation={<div className={styles.mobileNavigation}>{nav}</div>} productSwitch={{href:studioHref(),label:STUDIO_LABEL[locale]}} utilities={utilities}>
    <div className={styles.stage}><div className={styles.breadcrumb} aria-label="Breadcrumb"><Home/><ChevronRight/><span>{activeItem?.title[locale]??"Observatory"}</span></div><div className={styles.content} id="main-content" tabIndex={-1}>{children}</div></div>
  </ProductShell>;
}
