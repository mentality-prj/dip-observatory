...
const copy = {
  en: {
    ...
    subtitle:
      'Observatory is the inspection surface for QDIP decisions. Open a working application, change the context and inspect the recommendation, alternatives, constraints and evidence.',
    demos: 'Live decision applications',
    demosBody:
      'Different domains, the same decision grammar. Each application exposes how context becomes alternatives, evaluation and an explainable recommendation.',
  ...
  uk: {
    ...
    subtitle:
      'Observatory — простір перевірки рішень QDIP. Відкрийте робочий застосунок, змініть контекст і дослідіть рекомендацію, альтернативи, обмеження та докази.',
    demos: 'Робочі decision applications',
    demosBody:
      'Різні домени, однакова логіка рішення. Кожен застосунок показує, як контекст перетворюється на альтернативи, оцінювання та пояснювану рекомендацію.',
  ...
  pl: {
    ...
    subtitle:
      'Observatory to warstwa inspekcji decyzji QDIP. Otwórz działającą aplikację, zmień kontekst i sprawdź rekomendację, alternatywy, ograniczenia oraz dowody.',
    demos: 'Działające aplikacje decyzyjne',
    demosBody:
      'Różne domeny, ta sama logika decyzji. Każda aplikacja pokazuje, jak kontekst przechodzi w alternatywy, ocenę i wyjaśnialną rekomendację.',
            ...
            {useCases.map((useCase) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="mt-8 text-2xl font-semibold tracking-[-.025em] md:text-3xl">
        ...
        <section className="mt-16 border-t border-white/10 pt-10" aria-labelledby="observatory-inspect">
          ...
          <div className="mt-6 grid gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 md:grid-cols-2 xl:grid-cols-4">
            {t.questions.map(([title, body], index) => (
              <div key={title} className="bg-slate-950/80 p-6">
