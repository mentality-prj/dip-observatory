...
export function proxy(request:NextRequest){const host=requestHost(request),pathname=request.nextUrl.pathname;
 ...
 const explicit=pathname.match(/^\/platform\/(en|uk|pl)(\/.*)?$/);if(isMarketingHost(host)&&explicit&&MARKETING_LOCALES.has(explicit[1])){const u=request.nextUrl.clone();u.pathname=`/${explicit[1]}${explicit[2]??""}`;return NextResponse.redirect(u)}
