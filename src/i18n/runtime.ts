...
export function createTranslator(locale: Locale, namespace = ''): Translator {
  ...
  t.raw = <T = MessageValue>(key: string) => rawMessage<T>(locale, fullKey(key))
  t.has = (key: string) => resolvePath(catalogs[locale] as unknown as MessageValue, fullKey(key)) !== undefined
  return t
}
