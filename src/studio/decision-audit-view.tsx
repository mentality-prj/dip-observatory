...
export function DecisionAuditView({ initialId }: { initialId?: string }) {
  ...
  return <ProductShell theme="cyan" className="observatory-shell observatory-audit min-h-screen text-white" href={observatoryHref()} product="Observatory" productSwitch={{ href: studioHref(), label: "Open Studio" }}>
    ...
    <main id="main-content" tabIndex={-1} className="observatory-audit-main studio-main"><h1>Decision audit</h1><p>Historical decisions and alternative comparisons.</p>
      ...
      {!loading && !audits.length && !selected && !error && <Card><CardContent>No decisions have been evaluated yet.</CardContent></Card>}
      {audits.length > 0 && <label className="studio-field">Execution<select className="ds-select" value={selected?.decision_id ?? ""} onChange={(event) => { setSelected(audits.find((audit) => audit.decision_id === event.target.value) ?? null); setMessage(""); }}>{selected && !audits.some((audit) => audit.decision_id === selected.decision_id) && <option value={selected.decision_id}>{selected.profile.name} · {selected.timestamp}</option>}{audits.map((audit) => <option key={audit.decision_id} value={audit.decision_id}>{audit.profile.name} · {audit.timestamp} · {audit.status}</option>)}</select></label>}
      {selected && <><Card><CardHeader><div className="studio-card-heading"><CardTitle>{selected.profile.name}</CardTitle><Badge>{selected.status}</Badge></div></CardHeader><CardContent><p>Selected alternative: <strong>{selected.selected_alternative ?? "None"}</strong> · Profile version {selected.profile_version}</p>{selected.explanation.map((line) => <p key={line}>{line}</p>)}</CardContent></Card>
        <StudioUseCasePanel audit={selected} />
        {selected.dimension_results.map((alternative) => <Card key={alternative.alternative_id}><CardHeader><CardTitle>{alternative.alternative_id}</CardTitle></CardHeader><CardContent>{alternative.dimensions.map((dimension) => <details key={dimension.dimension_id}><summary>{dimension.dimension_id} · {dimension.status}{dimension.blocking && " · blocking"}{dimension.score !== null && ` · ${dimension.score.toFixed(3)}`}</summary><p>{dimension.explanation.join(" ")}</p>{dimension.required_actions.length > 0 && <p>Required actions: {dimension.required_actions.join(", ")}</p>}<pre>{JSON.stringify({ value: dimension.value, evidence: dimension.evidence, findings: dimension.findings }, null, 2)}</pre></details>)}</CardContent></Card>)}
