...
import { Badge, Button, Card, CardContent, CardHeader, CardTitle, Checkbox, Disclosure, Input, Select } from "@/design-system";
...
export function ProfileEditor({ initial, existing, plugins, dimensions, onSave, filter, section }: {
  ...
  return <form onSubmit={async (event) => {
    ...
    <Card><CardHeader><div className="studio-card-heading"><CardTitle>{existing ? "Edit profile" : "Create profile"}</CardTitle>
      ...
      <label className="studio-check"><Checkbox checked={profile.active} onChange={(e) => setProfile({ ...profile, active: e.target.checked })} />Activate after validation</label></>}
    </CardContent></Card>
    ...
    {(!section || !["overview", "alternatives"].includes(section)) && <Card><CardHeader><CardTitle>Decision dimensions</CardTitle><p>Select relevant dimensions, pin their contracts, and configure evaluation.</p></CardHeader><CardContent>
      <div className="studio-grid">{ids.filter((id) => !filter || id === filter).map((id) => {
        ...
        return <label className="studio-check" key={id}><Checkbox checked={selected} onChange={(event) => {
      ...
      {profile.dimensions.map((item, index) => {
        ...
        return <section key={item.dimension_id} className="studio-schema">
          ...
          <label className="studio-check"><Checkbox checked={item.required} onChange={(e) => changeDimension(index, { required: e.target.checked })} />Required</label>
    ...
    {(!section || section === "overview") && <Disclosure className="studio-schema"><summary>Runtime context schema</summary>
