...
export function ProfileEditor({
  ...
  function changeDimension(index: number, patch: Partial<ProfileDimension>) {
    setProfile({
      ...profile,
      dimensions: profile.dimensions.map((item, i) => (i === index ? { ...item, ...patch } : item)),
    })
  }
            ...
            <>
                  ...
                  <Select
                    ...
                    onChange={(e) => {
                      ...
                      setProfile({
                        ...
                        plugin_version: fresh.plugin_version,
                        capability_id: fresh.capability_id,
                        capability_version: fresh.capability_version,
                  ...
                  <Select
                    ...
                    onChange={(e) =>
                      setProfile({
                        ...
                        capability_version: plugin?.capability_versions[e.target.value] ?? '',
                        dimensions: profile.dimensions.map((d) => ({ ...d, binding_id: null, binding_version: null })),
                      })
                    }
                  >
              ...
              {plugin &&
                (profile.plugin_version !== plugin.version ||
                  profile.capability_version !== plugin.capability_versions[profile.capability_id]) && (
                  <Button
                    type="button"
                    variant="secondary"
                    onClick={() =>
                      setProfile({
                        ...
                        capability_version: plugin.capability_versions[profile.capability_id] ?? '',
                      })
                    }
                  >
            ...
            {profile.alternatives.map((alternative, index) => (
              <div key={index} className="studio-schema">
                  ...
                  <label className="studio-field">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setProfile({
                          ...profile,
                          alternatives: profile.alternatives.map((a, i) =>
                ...
                <JsonField
                  ...
                  onChange={(attributes) =>
                    setProfile({
                      ...profile,
                      alternatives: profile.alternatives.map((a, i) =>
                        i === index ? { ...a, attributes: attributes as Record<string, unknown> } : a
                      ),
                    })
                  }
                />
                <Button
                  type="button"
                  variant="secondary"
                  disabled={profile.alternatives.length === 1}
                  onClick={() =>
              ...
              onClick={() =>
                setProfile({
                  ...
                  alternatives: [
                    ...
                    { id: `alternative-${profile.alternatives.length + 1}`, label: '', attributes: {} },
                  ],
                })
              }
            >
            ...
            <div className="studio-grid">
              {ids
                ...
                .map((id) => {
                  ...
                  return (
                    <label className="studio-check" key={id}>
                      <Checkbox
                        checked={selected}
                        onChange={(event) => {
                          if (!event.target.checked)
                            setProfile({
                              ...profile,
                              dimensions: profile.dimensions.filter((d) => d.dimension_id !== id),
                            })
                          else {
                            const binding = bindings.find(
                              (b) => b.dimension_id === id && b.capability_id === profile.capability_id && b.enabled
                            )
                            setProfile({
                              ...profile,
                              dimensions: [
                                ...profile.dimensions,
                                {
                                  ...
                                  required: true,
                                  weight: definition.type === 'rules' ? 0 : 1,
                                  configuration: {},
            ...
            {profile.dimensions.map((item, index) => {
              ...
              return (
                <section key={item.dimension_id} className="studio-schema">
                        ...
                        {dimensions
                          ...
                          .map((d) => (
                            <option key={d.version} value={d.version}>
                              {d.version} · {d.evaluator_id}@{d.evaluator_version}
                            </option>
                          ))}
                      </Select>
                    </label>
                    {definition?.type !== 'rules' && (
                      <label className="studio-field">
                        ...
                        <Input
                          ...
                          required
                          value={item.weight}
                          onChange={(e) => changeDimension(index, { weight: e.target.valueAsNumber })}
                        />
                      </label>
                    )}
                    <label className="studio-field">
                      ...
                      >
                        ...
                        {bindings
                          .filter(
                            (b) => b.dimension_id === item.dimension_id && b.capability_id === profile.capability_id
                          )
                          .map((b) => (
                            <option key={b.id} value={`${b.id}|${b.version}`} disabled={!b.enabled}>
                              {b.source_path} → {b.dimension_id} · {b.version}
                            </option>
                          ))}
                      </Select>
                    </label>
                  ...
                  {definition && (
                    <SchemaField
                      key={`${item.dimension_id}-${item.version}`}
                      schema={definition.configuration_schema}
                      value={item.configuration}
                      onChange={(configuration) =>
                        changeDimension(index, { configuration: configuration as Record<string, unknown> })
                      }
                    />
                  )}
                </section>
      ...
      {(!section || section === 'overview') && (
        <Disclosure className="studio-schema">
          ...
          <JsonField
            ...
            onChange={(schema) => setProfile({ ...profile, context_schema: schema as Profile['context_schema'] })}
          />
        </Disclosure>
      )}
      {error && (
