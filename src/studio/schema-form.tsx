...
export function schemaDefault(schema: JsonSchema): unknown {
  ...
  if (schema.type === 'object')
    return Object.fromEntries(
      (schema.required ?? []).map((key) => [key, schemaDefault(schema.properties?.[key] ?? {})])
    )
...
export function JsonField({
  ...
  return (
    <label className="studio-field" htmlFor={id}>
      ...
      />
      {error && (
        <span role="alert" className="studio-error">
          {error}
        </span>
      )}
    </label>
    ...
    return (
      <fieldset className="studio-schema">
        ...
        {Object.entries(schema.properties).map(([key, child]) => {
          ...
          return (
            <div key={key} className="studio-schema-property">
              {!required && (
                <label className="studio-check">
                  <Checkbox
                    ...
                    onChange={(event) => {
                      const next = { ...object }
                      if (event.target.checked) next[key] = schemaDefault(child)
                      else delete next[key]
                      onChange(next)
                    }}
                  />
                  {child.title ?? key.replaceAll('_', ' ')}
                </label>
              )}
              {(present || required) && (
                <SchemaField
                  ...
                  value={object[key]}
                  depth={depth + 1}
                  label={child.title ?? key.replaceAll('_', ' ')}
