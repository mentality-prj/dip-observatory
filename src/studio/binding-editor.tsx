...
export function BindingEditor({ plugin, dimensions }: { plugin: Plugin; dimensions: Dimension[] }) {
  ...
  return <form onSubmit={async (event) => {
    ...
    {bindings.map((binding, index) => <Card key={index}>
      <CardHeader><CardTitle>{outputLabel(binding.capability_id, binding.source_path)} → {dimensions.find((d) => d.id === binding.dimension_id)?.name ?? binding.dimension_id}</CardTitle>
        <p>Pinned contracts: plugin {binding.plugin_version} · capability {binding.capability_version}</p></CardHeader>
      <CardContent>
      {(binding.plugin_version !== plugin.version || binding.capability_version !== plugin.capability_versions[binding.capability_id]) &&
        <Button variant="secondary" type="button" onClick={() => update(index, {
      ...
      <div className="studio-grid">
        ...
        <label className="studio-field">Available output<select className="ds-select" value={`${binding.capability_id}|${binding.dimension_id}|${binding.source_path}`} onChange={(e) => {
        ...
        <label className="studio-field">Dimension version<select className="ds-select" value={binding.dimension_version} onChange={(e) => update(index, { dimension_version: e.target.value })}>
      ...
      <Button variant="secondary" type="button" onClick={() => setBindings(bindings.filter((_, i) => i !== index))}>Remove binding</Button>
      </CardContent>
    </Card>)}
    <div className="studio-toolbar">
      <Button type="button" variant="secondary" disabled={!plugin.dimension_outputs.length} onClick={() => {
