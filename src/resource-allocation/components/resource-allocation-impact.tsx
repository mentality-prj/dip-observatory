...
function Impact({ label, current, recommended, format, inverse = false, unavailable }: { label: string; current?: number; recommended: number; format?: "pct"; inverse?: boolean; unavailable: string }) {
  ...
  if (current === undefined) return <div className="bg-white p-4"><div className="text-xs leading-4 text-black/45">{label}</div><div className="mt-4 text-2xl font-black">{render(recommended)}</div><div className="mt-2 text-xs text-black/35">{unavailable}</div></div>;
  ...
  return <div className="bg-white p-4"><div className="text-xs leading-4 text-black/45">{label}</div><div className="mt-4 flex items-end gap-2"><span className="text-sm text-black/35">{render(current)}</span><span className="text-black/20">→</span><b className="text-2xl">{render(recommended)}</b></div><div className={`mt-2 flex items-center gap-1 text-xs font-bold ${favorable === true ? "text-emerald-700" : favorable === false ? "text-red-700" : "text-black/35"}`}>{Math.abs(delta) < 0.0001 ? <span>0</span> : <><Icon className="h-3.5 w-3.5"/>{deltaText}</>}</div></div>;
}
